...
function toHourString(d: Date) {
  // "YYYY-MM-DD HH:00" (matches your SQL DATE_FORMAT)
  return `${d.getFullYear()}-${pad2(d.getMonth() + 1)}-${pad2(d.getDate())} ${pad2(d.getHours())}:00`;
}

function parseHourString(s: string): Date | null {
  // expects "YYYY-MM-DD HH:00" or "YYYY-MM-DD HH:00:00"
  const base = String(s || "").trim();
  if (!base) return null;
  const cleaned = base.replace("T", " ").split(".")[0];
...
export default function AnalyticsPage() {
  ...
  // 🔎 single search box for fixture filtering
  const [fixtureSearch, setFixtureSearch] = useState("");

  const [lookbackHours, setLookbackHours] = useState<number>(168);
  ...
  /**
   * Chart data with a forecast trend line + vertical ETA markers.
   * We keep XAxis as category (string) to avoid big refactor.
   * We add extra points at ETA hours (rounded to hour) so:
   *  - forecast line can be drawn (connectNulls)
   *  - ReferenceLine x=... can match an existing x value
   */
  const chartModel = useMemo(() => {
    ...
    if (!pts.length) {
      return {
        data: [] as Array<any>,
        etaWarnX: null as string | null,
        etaLimitX: null as string | null,
      };
    }

    // base time = last point timestamp (best for “from latest known sample”)
    const last = pts[pts.length - 1];
    const lastD = parseHourString(last.sample_ts) || new Date();
    const lastX = toHourString(lastD);

    // normalize last sample_ts formatting (so markers match)
    pts[pts.length - 1] = { ...last, sample_ts: lastX };

    const etaWarn = forecast?.eta_warning_hours;
    const etaLimit = forecast?.eta_limit_hours;

    const etaWarnD =
      typeof etaWarn === "number" && Number.isFinite(etaWarn) && etaWarn > 0
        ? new Date(lastD.getTime() + etaWarn * 3600 * 1000)
        : null;

    const etaLimitD =
      typeof etaLimit === "number" && Number.isFinite(etaLimit) && etaLimit > 0
        ? new Date(lastD.getTime() + etaLimit * 3600 * 1000)
        : null;

    const etaWarnX = etaWarnD ? toHourString(etaWarnD) : null;
    const etaLimitX = etaLimitD ? toHourString(etaLimitD) : null;

    // We'll draw a forecast trend line using a separate key "forecast_contacts"
    // only at lastX and at threshold points.
    const baseContacts = Number(last.contacts ?? currentContacts ?? 0);

    const extras: any[] = [];

    // always set forecast value on last sample
    // (we will later map pts to include forecast_contacts = null except last)
    if (etaWarnX && typeof warningLine === "number" && warningLine > 0) {
      extras.push({
        sample_ts: etaWarnX,
        contacts: warningLine,
        warning_at: typeof warningLine === "number" ? warningLine : 0,
        contacts_limit: typeof limitLine === "number" ? limitLine : 0,
        resets: Number(selectedFixture?.resets ?? 0),
        forecast_contacts: warningLine,
        _isForecastPoint: true,
      });
    }

    if (etaLimitX && typeof limitLine === "number" && limitLine > 0) {
      extras.push({
        sample_ts: etaLimitX,
        contacts: limitLine,
        warning_at: typeof warningLine === "number" ? warningLine : 0,
        contacts_limit: typeof limitLine === "number" ? limitLine : 0,
        resets: Number(selectedFixture?.resets ?? 0),
        forecast_contacts: limitLine,
        _isForecastPoint: true,
      });
    }

    // map existing points to include forecast_contacts only for last point
    const withForecastKey = pts.map((p, idx) => ({
      ...p,
      // keep default lines available
      warning_at: Number(p.warning_at ?? warningLine ?? 0),
      contacts_limit: Number(p.contacts_limit ?? limitLine ?? 0),
      forecast_contacts: idx === pts.length - 1 ? baseContacts : null,
      _isForecastPoint: false,
    }));

    // merge and sort by sample_ts string (works because "YYYY-MM-DD HH:00" sorts)
    const merged = [...withForecastKey, ...extras].sort((a, b) =>
      String(a.sample_ts).localeCompare(String(b.sample_ts))
    );

    // de-duplicate by sample_ts keeping the most “complete” record
    const byTs = new Map<string, any>();
    for (const row of merged) {
      const key = String(row.sample_ts);
      ...
      if (!prev) {
        ...
        byTs.set(key, {
          ...prev,
          ...row,
          // keep contact value from real samples if present
          contacts: prev._isForecastPoint ? row.contacts : prev.contacts,
          // keep forecast_contacts if present
          forecast_contacts: row.forecast_contacts ?? prev.forecast_contacts ?? null,
          _isForecastPoint: prev._isForecastPoint && row._isForecastPoint,
        });
      }
    }
    ...
    return { data, etaWarnX, etaLimitX };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [series, forecast, warningLine, limitLine, selectedFixture?.resets, currentContacts]);
    ...
    <div className="mx-4 my-4">
      ...
      >
        ...
        <div style={{ gridColumn: "span 2", minWidth: 220 }}>
          <div style={{ fontSize: 12, opacity: 0.85, color: "#fff", display: "flex", gap: 6, alignItems: "center" }}>
            Plant <span title="Select plant (non-admins usually get only one).">?</span>
          </div>
          <select
            value={selectedPlant}
            onChange={(e) => {
        ...
        <div style={{ gridColumn: "span 4", minWidth: 360 }}>
          <div style={{ fontSize: 12, opacity: 0.85, color: "#fff", display: "flex", gap: 6, alignItems: "center" }}>
            Fixture search <span title="Filters the fixture dropdown by project / adapter / fixture type.">?</span>
          </div>
          <input
            value={fixtureSearch}
          ...
          />
          {selectedFixture ? (
            <div style={{ fontSize: 12, color: "#fff", opacity: 0.85, marginTop: 6 }}>
        ...
        {/* Fixture dropdown */}
        <div style={{ gridColumn: "span 3", minWidth: 360 }}>
          <div style={{ fontSize: 12, opacity: 0.85, color: "#fff", display: "flex", gap: 6, alignItems: "center" }}>
            Fixture <span title="Choose the fixture you want to analyze.">?</span>
          </div>
          <select
        ...
        {/* Lookback */}
        <div style={{ gridColumn: "span 1", minWidth: 190 }}>
          <div style={{ fontSize: 12, opacity: 0.85, color: "#fff", display: "flex", gap: 6, alignItems: "center" }}>
            Forecast lookback <span title="How far back the system looks to compute burn rate / ETA. Larger = smoother.">?</span>
          </div>
          <select
            value={lookbackHours}
        ...
        <div style={{ gridColumn: "span 1", minWidth: 190 }}>
          <div style={{ fontSize: 12, opacity: 0.85, color: "#fff", display: "flex", gap: 6, alignItems: "center" }}>
            Series range <span title="Chart range. Bigger ranges can look flatter if contacts are huge.">?</span>
          </div>
          <select
            value={seriesHours}
        ...
        {/* Refresh */}
        <div style={{ gridColumn: "span 1", minWidth: 220, justifySelf: "end" }}>
          <button
            onClick={loadAnalytics}
            disabled={loading || !selectedFixture}
            title="Reload all analytics for the selected fixture."
            style={{
              padding: "10px 14px",
              width: "100%",
              borderRadius: 10,
          ...
          {lastLoadedAt ? (
            <div style={{ fontSize: 12, color: "#fff", opacity: 0.85, marginTop: 6, textAlign: "right" }}>
      ...
      <div className="analytics-card p-3 rounded text-white" style={{ marginBottom: 14 }}>
        <div style={{ fontWeight: 700, marginBottom: 8 }}>Contacts (hourly) + Forecast</div>

        {/* fixed-height container -> avoids width(-1)/height(-1) issues */}
        <div style={{ width: "100%", height: 360, minWidth: 0 }}>
          {mounted && chartModel.data.length ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartModel.data}>
                <CartesianGrid />
                <XAxis dataKey="sample_ts" tick={{ fontSize: 11 }} minTickGap={18} />
                ...
                {chartModel.etaWarnX ? (
                  <ReferenceLine
                    x={chartModel.etaWarnX}
                    stroke="#f0ad4e"
                    strokeDasharray="3 6"
                    label={{ value: `ETA Warning (${chartModel.etaWarnX})`, position: "top", fill: "#f0ad4e", fontSize: 11 }}
                  />
                ) : null}
                {chartModel.etaLimitX ? (
                  <ReferenceLine
                    x={chartModel.etaLimitX}
                    stroke="#d9534f"
                    strokeDasharray="3 6"
                    label={{ value: `ETA Limit (${chartModel.etaLimitX})`, position: "top", fill: "#d9534f", fontSize: 11 }}
                  />
                ) : null}

                {/* Actual contacts line */}
                <Line
                  type="monotone"
                  dataKey="contacts"
                  name="Contacts"
                ...
                {/* Forecast trend line (connects last point -> thresholds) */}
                <Line
                  type="monotone"
                  dataKey="forecast_contacts"
                  name="Forecast trend"
                  dot={false}
                  connectNulls
                  stroke="#b56bff"
        ...
        <div className="label" style={{ marginTop: 8, opacity: 0.9 }}>
          Tip: If the curve looks flat because contacts are huge, increase Series range and use longer Lookback for smoother ETA.
        </div>
      </div>

      {/* Daily overview + Demand (kept as-is for now, per your note) */}
